...
template <class array2d>
inline
void logFunc(const array2d& u,const Constants& c, uint index);
...
solve(array2d& u, const Constants& c, bool iterator, bool parallel)
{
  ...
  double* p = (double *) malloc(u.size() * sizeof(double));
  ...
  for (t = 0; t < c.tfinal; t += c.dt) {
    ...
    //compress and decompress to introduce error.
  	//are you plan to compress the error in similar rate?
    //index++;
...
int main(int argc, char* argv[])
{
  ...
  printf("%10f", c.pi * 100000000);

  //high precision solution
  long double * hu = (long double *) malloc(c.nx * c.ny * sizeof(long double));
  
  for(int i = 0; i < c.nx * c.ny; i++)
  	hu[i] = 0;
  
  HighResolutionConstants hc(nx, ny, nt);
  HighResolution_Solver(hu, hc);

  //compress or not compress solution
  double sum;
  double err;
  if (compression) {
    ...
    long double * lu = (long double *) malloc(c.nx * c.ny * sizeof(long double));
    for(int i = 0; i < hc.ny; i++)
      for(int j = 0; j < hc.nx; j++)
        lu[i * hc.nx + j] = u[i * hc.nx + j];

    std::cout << hu[c.x0+ c.y0 * c.ny] -lu[c.x0+ c.y0 * c.ny]<< std::endl; 
    long double maxvalue = -1;
    long double minvalue = 1;
    for(int i = 0; i < hc.ny; i++)
      for(int j = 0; j < hc.nx; j++){
        if(maxvalue < hu[i * hc.nx + j] - lu[i * hc.nx + j])
          maxvalue = hu[i * hc.nx + j] - lu[i * hc.nx + j];

        if(minvalue > hu[i * hc.nx + j] - lu[i * hc.nx + j])
          minvalue = hu[i * hc.nx + j] - lu[i * hc.nx + j];
      }
 
    std::cout << "the maximum L_infinity error" << std::endl;
    //printf("%.100f \n", maxvalue);
    //printf("%.100f \n", minvalue);
